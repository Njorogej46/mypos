...
class Stock extends CI_Controller
{
    ...
    public function process()
    {
        if (isset($_POST['in_add'])) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->session->set_flashdata('message', 'Stock-In data successfully added');
                redirect('stock/in');
            }
        } else {
            $post = $this->input->post(null, true);
            ...
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('message', 'Stock-Out data added successfully');
                redirect('stock/out');
            }
        }
    }
    ...
    public function stock_in_del()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('message', 'Stock-In data has been deleted successfully');
            redirect('stock/in');
        }
    }
    ...
    public function stock_out_del()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('message', 'Stock-Out data has been deleted successfully');
            redirect('stock/out');
        }
    }
}
